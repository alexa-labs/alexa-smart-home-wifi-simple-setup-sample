...
class ApiHandlerEndpoint:
    ...
    def Provision(self, request):
        try:
            ...
            print(json_object)
            wssSessionToken, wssSessionTokenSignature, deviceIdentifier = '','',''
            if 'data' in json_object:
                wssSessionToken = json_object['data']['wssSessionToken']
                wssSessionTokenSignature = json_object['data']['wssSessionTokenSignature']
                deviceIdentifier = json_object['data']['deviceIdentifier']
            else:
                wssSessionToken = json_object['device']['sessionToken']
                wssSessionTokenSignature = json_object['device']['signature']
                deviceIdentifier = json_object['device']['id']
                #namingCategory = json_object['device']['namingCategories']

            event_response =  ApiHandlerEvent.send_sdae_event(wssSessionToken,wssSessionTokenSignature , deviceIdentifier)
            return event_response
        except KeyError as key_error:
            return "KeyError: " + str(key_error)

    # Create a device with association to customer
    def createDeviceFromCDAE(self,payload, userId, friendlyName):
        try:
            ...
            endpoint_details.user_id = userId  # Expect a Profile
            endpoint_details.sku = "LI00"  # A custom endpoint type, ex: SW0
            endpoint_details.friendly_name = friendlyName
            endpoint_details.manufacturer_name = "Sample Manufacturer"
            endpoint_details.description = "Wifi Simple Setup Device"
            endpoint_details.display_categories =["LIGHT"]
