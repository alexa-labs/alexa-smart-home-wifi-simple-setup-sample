...
class ApiHandlerDirective:
    ...
    def process(self, request, client_id, client_secret, redirect_uri):
        ...
        if json_body:
            ...
            if namespace == "Alexa.SimpleSetup":
                print("CDAE - Simple Setup Payload: " + str(json_object))
                name = json_object['directive']['header']['name']
                #instance = json_object['directive']['header']['instance']
                token = json_object['directive']['payload']['scope']['token']
                response_user_id = json.loads(ApiAuth.get_user_id(token).read().decode('utf-8'))
                if 'error' in response_user_id:
                    print('ERROR api_handler_directive.process.SimpleSetup.user_id: ' + response_user_id['error_description'])
                user_id = response_user_id['user_id']
                print('LOG api_handler_directive.process.SimpleSetup.user_id:', user_id)
                api_handler = ApiHandlerEndpoint()
                endpoint = api_handler.createDeviceFromCDAE(json_object,user_id, 'First Bulb')
                print(response)
                alexa_response = AlexaResponse(token=token, endpoint_id=endpoint['id'])
                alexa_response.add_context_property(
